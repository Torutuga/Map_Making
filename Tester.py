...
proof = open("proof.txt", "a")
for drops in range(100):
    proof.write(f"+++ Drop Number {drops}\n\n")
    drop = np.array([random.uniform(2.0,float(height-1)),random.uniform(2.0,float(width-1))])
    proof.write(f"Creation Coordinates: {drop}\n")
    wtr = 1.0
    sed = 0.0
    vel = 1.0
    dir = np.array([0,0])
    ...
    for steps in range(max_path):
        proof.write(f"-------------Step {steps}\n")
        g, h = GRAD_HIGT(height, width, drop, blue)
        proof.write(f"Gradient: {g}\n")
        proof.write(f"Height: {h}\n")
        dir = dir*inertia - g*(1-inertia)
        nrml = dir / math.sqrt(dir[0]**2 + dir[1]**2)
        proof.write(f"Normalized Dir: {nrml}\n")
        drop_new = drop + nrml
        proof.write(f"New Pos: {drop_new}\n")

        if drop_new[0] > height-1 or drop_new[1] > width-1:
            proof.write("Out of Bounds.\n")
            break
        ...
        proof.write(f"Height at New Position: {h_new}\n")
        h_dif = h_new - h
        proof.write(f"Height Difference: {h_dif}\n")
        c = np.maximum(-h_dif, min_slope)*vel*wtr*capacity
        proof.write(f"Capacity: {c}\n")

        if sed > c:
            proof.write(f"Sediment over Capacity\n")
            total_depo = (sed-c)*deposition
            sed -= total_depo
            proof.write(f"Sediment: {sed} \n") 
            Depositer(blue, drop, total_depo)
        else:
            if h_dif > 0.0:
                proof.write(f"Going Uphill\n") 
                total_depo = min(sed, h_dif)
                sed -= total_depo
                proof.write(f"Sediment: {sed} \n") 
                Depositer(blue, drop, total_depo)
            else:
                proof.write(f"Going Downhill\n") 
        ...
        try:
            ...
            break
        proof.write(f"New Velocity: {vel}\n")
        wtr = wtr*(1-evaporation)
        proof.write(f"New Water: {wtr}\n")
        hop = H_BINT(int(drop[1]), int(drop[0]), drop, blue)
        proof.write(f"Height at Old Position: {hop}\n")
        proof.write(f"New Height Diff: {h_new-hop}\n")
        drop = drop_new
    proof.write("\n")
